  ...
  /*! \brief Объединение, определяющее внутренние данные генератора */
   union {
     ...
     /*! \brief Указатель на произвольную структуру данных. */
       ak_pointer ctx;
     /*! \brief Внутреннее состояние генератора Вихря Мерсенна 64 */
       struct {
	  ak_uint64 mt[312];
	  int index;
	  ak_uint64 value;} MT_64;
   } data;
 };

/* ----------------------------------------------------------------------------------------------- */
/*! \brief Инициализация контекста генератора псевдо-случайных чисел Вихрь Мерсенна 64. */
  dll_export int ak_random_create_mt19937_64( ak_random );
